...
Base SAS is enough: no SAS/STAT, no SAS/ACCESS, no PROC FCMP. The port has not
yet been run on a SAS installation, so run the bundled self-test first; it
checks the block table against reference output generated by the R package:

```sas
%include "&PMDIR/physmerge_selftest.sas";   /* prints SELFTEST: PASS */
```

`sas/README.md` has the macro reference and the two SAS-specific traps worth
knowing about (a missing value compares below every number, and `BEST32.` does
not read p-values far below 1e-300 reliably).
...
- `sas/README.md`: the SAS macros, the self-test, and the SAS-specific traps
- `TECHNICAL_SPEC.txt`: algorithm, data contract, edge cases (in Chinese)
- `tests/qa/FINDINGS.txt`: the edge cases the three implementations were checked against
